    ...
    private val helper = ItemTouchHelper(object : ItemTouchHelper.SimpleCallback(ItemTouchHelper.UP or ItemTouchHelper.DOWN, ItemTouchHelper.LEFT or ItemTouchHelper.RIGHT) {
        ...
        override fun onChildDraw(c: Canvas, recyclerView: RecyclerView, viewHolder: RecyclerView.ViewHolder, dX: Float, dY: Float, actionState: Int, isCurrentlyActive: Boolean) {
            if (actionState == ItemTouchHelper.ACTION_STATE_SWIPE) {
                ...
                val icon: Bitmap = Utils.drawableToBitmap(drawable)
                ...
                if (dX > 0) {
                    ...
                    c.drawBitmap(icon,
                            itemView.left.toFloat() + Utils.dpAsPx(this@TimesConfigureActivity, 16),
                    ...
                    c.drawBitmap(icon,
                            itemView.right.toFloat() - Utils.dpAsPx(this@TimesConfigureActivity, 16) - icon.width,
                            itemView.top.toFloat() + (itemView.bottom.toFloat() - itemView.top.toFloat() - icon.height.toFloat()) / 2,
                            p)
                }
